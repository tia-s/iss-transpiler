'====================================================================================================
'	Test#: 		Transaction Monitoring Script - Personal Cash Out
'	Risk:		
' 	Objective:	Identify Individual accounts with debit transactions over threhold in a day
' 	Frequency:	Daily
' 	Last Modified:	29/09/2014 12:10:34
'====================================================================================================
...
Const RESULT_FILENAME = "Transaction Monitoring Script - Personal Cash Out"
Const scriptname_log ="Transaction Monitoring Script - Personal Cash Out.iss"
Global errors_string As String
Const division = "Sagicor"
Dim fso As Object
...
Sub Main
	...
	Call GetPersonalCashOut()
	Call ModifyFieldDetails()
	Call GetSummaryRecords()
	Call GetDetailRecords()
	Call ModifyFieldSummary()
	Call Export()	
...
'=====================================================================	
Function GetPersonalCashOut
If haveRecords("T24 Customer Transactions Complete.IMD") Then
	Set db = Client.OpenDatabase("T24 Customer Transactions Complete.IMD")
	Set task = db.Extraction
		task.IncludeAllFields
		dbName = "Personal Customer.IMD"
		task.AddExtraction dbName, "", "CUSTOMER_TYPE == ""P"""
...
' Cash Out
If haveRecords("Personal Customer.IMD") Then
	Set db = Client.OpenDatabase("Personal Customer.IMD")
	Set task = db.Extraction
	task.IncludeAllFields
	dbName = "Personal Cash Out.IMD"
	task.AddExtraction dbName, "", "@Match(TRANSACTION_CODE, ""CASH"", ""ATMC"", ""DBTC"") .AND. DEBIT_CREDIT == ""D"""
	task.CreateVirtualDatabase = False
	task.PerformTask 1, db.Count
	Set task = Nothing
	Set db = Nothing
...
Function ModifyFieldDetails
If haveRecords("Personal Cash Out.IMD")Then
	Set db = Client.OpenDatabase("Personal Cash Out.IMD")
...
If haveRecords("Personal Cash Out.IMD")Then
	...
	Set task = db.Summarization
		...
		dbName = "Personal Cash Out Summ.IMD"
	...
	Set db = Client.OpenDatabase("Personal Cash Out Summ.IMD")
	Set task = db.Extraction
		task.IncludeAllFields
		dbName = "Personal Cash Out Summary.IMD"
		task.AddExtraction dbName, "", "TRANSACTION_AMOUNT_SUM >" & e_Personal_Cash_Out_Thresh
		task.CreateVirtualDatabase = False
		task.PerformTask 1, db.Count
	Set task = Nothing
	Set db = Nothing
...
If haveRecords("Personal Cash Out.IMD") And haveRecords("Personal Cash Out Summary.IMD") Then
	Set db = Client.OpenDatabase("Personal Cash Out.IMD")
	Set task = db.JoinDatabase
		task.FileToJoin "Personal Cash Out Summary.IMD"
		task.IncludeAllPFields
		task.AddSFieldToInc "ACCOUNT_NUMBER"
		task.AddMatchKey "ACCOUNT_NUMBER", "ACCOUNT_NUMBER", "A"
		task.CreateVirtualDatabase = False
		dbName = "Personal Cash Out Results.IMD"
...
Function ModifyFieldSummary
If haveRecords("Personal Cash Out Summary.IMD") Then
	Set db = Client.OpenDatabase("Personal Cash Out Summary.IMD") 
	Set task = db.TableManagement
	Set field = db.TableDef.NewField
...
If haveRecords("Personal Cash Out Summary.IMD")  Then
	...
	Set task = db.ExportDatabase
		...
		task.AddFieldToInc "TRANSACTION_CURRENCY"
		eqn = ""
		task.PerformTask Client.WorkingDirectory &  "Reports\Personal Cash Out Summary.MDB", "Database", "MDB2000", 1, db.Count, eqn
	Set db = Nothing
End If
	Set task = Nothing
	
If haveRecords("Personal Cash Out Results.IMD") Then
	Set db = Client.OpenDatabase("Personal Cash Out Results.IMD")
	Set task = db.ExportDatabase
		task.AddFieldToInc "ACCOUNT_NUMBER"
		task.AddFieldToInc "CUSTOMER_NUMBER"
		task.AddFieldToInc "TRANSACTION_DATE"
		...
		eqn = ""
		task.PerformTask Client.WorkingDirectory & "Reports\Personal Cash Out Details.MDB", "Database", "MDB2000", 1, db.Count, eqn
	Set db = Nothing
End If
	Set task = Nothing
...
Function CleanUp
	DeleteFile("Personal Customer.IMD") 
	DeleteFile("Personal Cash Out.IMD")
	DeleteFile("Personal Cash Out Results.IMD")
	DeleteFile("Personal Cash Out Summary.IMD")
End Function

...
wd.extract("Personal_Cheque_In_Wire_Out", cols=['"SOURCE_ACCOUNT"', '"DEBIT_CREDIT"', '"CURRENCY"', '"OTHER_PARTY_ACC"', '"EXCLUDE_FROM_PROFILING"', '"DATE"', '"TRANSACTION_CODE"', '"AMOUNT"', '"ORIGINAL_AMOUNT"', '"REFERENCE"', '"DESCRIPTION"', '"FROM_CUSTOMER"', '"TO_ACCOUNT"', '"ROLE_TYPE"', '"CUSTOMER_NUMBER"', '"NAME"', '"COUNTRY_RESIDENCE"', '"COUNTRY_NATIONALITY"', '"DATEOFBIRTH"', '"ADDRESS_LINE_1"', '"ADDRESS_LINE_2"', '"ADDRESS_LINE_3"', '"ADDRESS_LINE_4"', '"ADDRESS_LINE_5"', '"GENDER"', '"CUSTOMER_TYPE"', '"BUSINESS_CODE"', '"SSN"', '"COUNTRY_BIRTH"', '"OCCUPATION"', '"IDENTIFICATION_TYPE"', '"IDENTIFICATION_OTHER"', '"IDENTIFICATION_NUMBER"', '"ISSUING_AUTHORITY"', '"CONTACT_NUMBER"', '"COUNTRY_RISK"', '"CUSTOMER_STATUS"', '"RISK_CATEGORY"', '"PEP"', '"ACCOUNT_TYPE"', '"STATUS"', '"ACCOUNT_NAME"', '"TRANSACTION_CURRENCY"', '"TRANSACTION_AMOUNT"', '"ORIGINAL_TRANSACTION_AMOUNT"', '"RUN_DATE"'])


# Get the first cheque in generated per account

def B_Get_First_ChequeIn_Per_Account():
if not wd.db("Personal_Cheque_In_Wire_Out").empty:
wd.open("Personal_Cheque_In_Wire_Out")
top recs: {'fields': ['"SOURCE_ACCOUNT"', '"DATE"'], 'keys': [('"SOURCE_ACCOUNT"', '"A"'), ('"DATE"', '"A"')], 'db_name': '"First Personal Cheque In"', 'output_file': dbName, 'no_of_recs': 1, 'virt_db': False, 'perf_task': ''}


# Link cheque In and Wire out to place the first Wire in date beside all the txns.

def C_Join_FirstChequeIn_To_AllChequeInWireOut():
if not wd.db("Personal_Cheque_In_Wire_Out").empty:
wd.open("Personal_Cheque_In_Wire_Out")
wd.join("Personal_Cheque_In_Wire_Out_First_ChequeIn", right=wd.db("First Personal Cheque In")[['"DATE"']], how=WI_JOIN_MATCH_ONLY, on=['"SOURCE_ACCOUNT"', '"SOURCE_ACCOUNT"', '"A"'])


# Get first Personal Wire out after first cheque in.

def D_Get_First_WireOut_After_CheckIn():
if not wd.db("Personal_Cheque_In_Wire_Out_First_ChequeIn").empty:
wd.open("Personal_Cheque_In_Wire_Out_First_ChequeIn")
top recs: {'fields': ['"SOURCE_ACCOUNT"', '"DATE"', '"SOURCE_ACCOUNT"', '"DATE"', '"DATE1"'], 'keys': [('"SOURCE_ACCOUNT"', '"A"'), ('"DATE"', '"A"'), ('"SOURCE_ACCOUNT"', '"A"'), ('"DATE"', '"A"')], 'db_name': '"Personal Wire Out After Cheque In"', 'output_file': dbName, 'no_of_recs': 1, 'virt_db': False, 'perf_task': ''}


# Link Wire In and Out to Wire out file

def E_Get_CheckIn_WireOut_Details():
if not wd.db("Personal_Cheque_In_Wire_Out").empty:
wd.open("Personal_Cheque_In_Wire_Out")
wd.join("Personal_Cheque_In_Wire_Out_Temp2", right=wd.db("Personal Wire Out After Cheque In")[['"DATE"', '"SOURCE_ACCOUNT"', '"DATE1"']], how=WI_JOIN_MATCH_ONLY, on=['"SOURCE_ACCOUNT"', '"SOURCE_ACCOUNT"', '"A"'])
# Ensure ONLY records that fall after the first Wire in are selected for the summary	

if not wd.db("Personal_Cheque_In_Wire_Out_Temp2").empty:
wd.open("Personal_Cheque_In_Wire_Out_Temp2")
wd.extract("Personal_Cheque_In_Wire_Out_Temp", cols=all)


def F_ModifyFieldDetails():
if not wd.db("Personal_Cheque_In_Wire_Out_Temp").empty:
wd.open("Personal_Cheque_In_Wire_Out_Temp")
...
if not wd.db("Personal_Cheque_In_Wire_Out_Temp").empty:
wd.open("Personal_Cheque_In_Wire_Out_Temp")
wd.summBy(""Personal_Cheque_In_Wire_Out_Summ"", ['"ACCOUNT_NUMBER"'], agg_funcs={key: ['sum'] if key != ""ACCOUNT_NUMBER"" else ['count'] for key in ['"TRANSACTION_AMOUNT"', '"ACCOUNT_NUMBER"']})
wd.renameCol(columns={""ACCOUNT_NUMBER"_count": "NO_OF_RECS"})
wd.join(""Personal_Cheque_In_Wire_Out_Summ"", right=wd.db(""Personal_Cheque_In_Wire_Out_Summ"_summ")[['"ACCOUNT_NAME"', '"ACCOUNT_TYPE"', '"ROLE_TYPE"', '"ACCOUNT_STATUS"', '"CUSTOMER_NUMBER"', '"CUSTOMER_NAME"', '"CUSTOMER_TYPE"', '"TRANSACTION_DATE"', '"RUN_DATE"', '"TRANSACTION_CURRENCY"']], how="left")
if not wd.db("Personal_Cheque_In_Wire_Out_Summ").empty:
wd.open("Personal_Cheque_In_Wire_Out_Summ")
wd.extract("Personal_Cheque_In_Wire_Out_Summary", cols=all)


def H_GetDetailRecords():
if not wd.db("Personal_Cheque_In_Wire_Out_Temp").empty:
wd.open("Personal_Cheque_In_Wire_Out_Temp")
wd.join("Personal_Cheque_In_Wire_Out_Details", right=wd.db("Personal Cheque In Wire Out Summary")[['"DATE"', '"SOURCE_ACCOUNT"', '"DATE1"', '"RUN_DATE"']], how=WI_JOIN_MATCH_ONLY, on=['"ACCOUNT_NUMBER"', '"ACCOUNT_NUMBER"', '"A"'])


def I_ModifyFieldSummary():
if not wd.db("Personal_Cheque_In_Wire_Out_Summary").empty:
wd.open("Personal_Cheque_In_Wire_Out_Summary")
...
if not wd.db("Personal_Cheque_In_Wire_Out_Details").empty:
wd.open("Personal_Cheque_In_Wire_Out_Details")
...
wd.delete("Personal Cheque In Wire Out")
wd.delete("First Personal Cheque In")
wd.delete("Personal Cheque In Wire Out First ChequeIn")
wd.delete("Personal Wire Out After Cheque In")
wd.delete("Personal Cheque In Wire Out Details")
wd.delete("Personal Cheque In Wire Out Summary")
wd.delete("Personal Cheque In Wire Out Summ")
wd.delete("Personal Cheque In Wire Out Temp")
wd.delete("Personal Cheque In Wire Out Temp2")
wd.delete("Personal Cheque In Wire Out Summary Final")

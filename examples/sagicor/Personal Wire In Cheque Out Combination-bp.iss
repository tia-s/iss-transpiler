'====================================================================================================
'	Test#: 		Personal Wire In Cheque Out Combination
'	Risk:		Personal customers may be receiving wire transfers and withdrawing cheques over the threshold.
' 	Objective:	Identify Personal customers that receive wires and withdraw cheques where the total is over 
'			the threshold.
' 	Frequency:	FT
...
Const RESULT_FILENAME = "Personal Wire In Cheque Out Combination"
Const scriptname_log ="Personal Wire In Cheque Out Combination.iss"
...
Sub Main
	...
	Client.CloseAll
	Z_CleanUp
	A_Get_WireIn_ChequeOut
	B_Get_First_WireIn_Per_Account
	C_Join_FirstWireIn_To_AllWireInChequeOut
	D_Get_First_ChequeOut_After_CheckIn
	E_Get_CheckIn_ChequeOut_Details
...
Function A_Get_WireIn_ChequeOut
...
'		task.AddFieldToInc "CUS_NUMBER"
		...
		dbName = "Personal Wire In Cheque Out.IMD"
		task.AddExtraction dbName, "", "((@Match(TRANSACTION_CODE, ""BNKC"", ""CHCK"") .AND. DEBIT_CREDIT == ""D"" ) .OR. (@Match(TRANSACTION_CODE, ""INTT"", ""WRTR"")   .AND. DEBIT_CREDIT == ""C""))  .AND. CUSTOMER_TYPE == ""P"""
...
Function B_Get_First_WireIn_Per_Account
If haveRecords("Personal Wire In Cheque Out.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out.IMD")
	Set task = db.TopRecordsExtraction
		task.AddFieldToInc "SOURCE_ACCOUNT"
		task.AddFieldToInc "DATE"
		task.AddKey "SOURCE_ACCOUNT", "A"
		task.AddKey "DATE", "A"
		task.Criteria = "@Match(TRANSACTION_CODE, ""INTT"", ""WRTR"")  .AND. DEBIT_CREDIT == ""C"""
		dbName = "First Personal Wire In.IMD"
...
Function C_Join_FirstWireIn_To_AllWireInChequeOut
If haveRecords("Personal Wire In Cheque Out.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out.IMD")
	Set task = db.JoinDatabase
   If haveRecords("First Personal Wire In.IMD") Then
		task.FileToJoin "First Personal Wire In.IMD"
		task.IncludeAllPFields
		task.AddSFieldToInc "DATE"
		task.AddMatchKey "SOURCE_ACCOUNT", "SOURCE_ACCOUNT", "A"
		task.CreateVirtualDatabase = False
		dbName = "Personal Wire In Cheque Out First WireIn.IMD"
...
Function D_Get_First_ChequeOut_After_CheckIn
If haveRecords("Personal Wire In Cheque Out First WireIn.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out First WireIn.IMD")
	Set task = db.TopRecordsExtraction
		task.AddFieldToInc "SOURCE_ACCOUNT"
		task.AddFieldToInc "DATE"
		...
		task.AddKey "SOURCE_ACCOUNT", "A"
		task.AddKey "DATE", "A"
		task.Criteria = "@Match(TRANSACTION_CODE, ""Cheque"", ""ATMC"", ""DBTC"") .AND. DEBIT_CREDIT == ""D"" .AND. DATE1 < Date"
		dbName = "Personal Cheque Out After Wire In.IMD"
		task.OutputFileName = dbName
		task.NumberOfRecordsToExtract = 1
		task.CreateVirtualDatabase = False
		task.PerformTask
...
Function E_Get_CheckIn_ChequeOut_Details
If haveRecords("Personal Wire In Cheque Out.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out.IMD")
	Set task = db.JoinDatabase
   If haveRecords("Personal Cheque Out After Wire In.IMD") Then
		task.FileToJoin "Personal Cheque Out After Wire In.IMD"
		task.IncludeAllPFields
		task.AddSFieldToInc "SOURCE_ACCOUNT"
		task.AddSFieldToInc "DATE1"
		task.AddMatchKey "SOURCE_ACCOUNT", "SOURCE_ACCOUNT", "A"
		task.CreateVirtualDatabase = False
		dbName = "Personal Wire In Cheque Out Temp2.IMD"
		task.PerformTask dbName, "", WI_JOIN_MATCH_ONLY
End If
	Set task = Nothing
	Set db = Nothing
...
If haveRecords("Personal Wire In Cheque Out Temp2.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out Temp2.IMD")
	Set task = db.Extraction
		task.IncludeAllFields
		dbName = "Personal Wire In Cheque Out Temp.IMD"
...
If haveRecords("Personal Wire In Cheque Out Temp.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out Temp.IMD")
	Set task = db.TableManagement
		Set field = db.TableDef.NewField
...
If haveRecords("Personal Wire In Cheque Out Temp.IMD") Then
	...
	Set task = db.Summarization
		...
		task.AddFieldToInc "TRANSACTION_CURRENCY"
		dbName = "Personal Wire In Cheque Out Summ.IMD"
...
If haveRecords("Personal Wire In Cheque Out Summ.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out Summ.IMD")
	Set task = db.Extraction
		task.IncludeAllFields
		dbName = "Personal Wire In Cheque Out Summary.IMD"
...
If haveRecords("Personal Wire In Cheque Out Temp.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out Temp.IMD")
	Set task = db.JoinDatabase
   If haveRecords("Personal Wire In Cheque Out Summary.IMD") Then
		task.FileToJoin "Personal Wire In Cheque Out Summary.IMD"
		task.IncludeAllPFields
		task.AddSFieldToInc "RUN_DATE"
		task.AddMatchKey "ACCOUNT_NUMBER", "ACCOUNT_NUMBER", "A"
		task.CreateVirtualDatabase = False
		dbName = "Personal Wire In Cheque Out Details.IMD"
		task.PerformTask dbName, "", WI_JOIN_MATCH_ONLY
End If
	Set task = Nothing
	Set db = Nothing
...
If haveRecords("Personal Wire In Cheque Out Summary.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out Summary.IMD") 
	Set task = db.TableManagement
	Set field = db.TableDef.NewField
...
If haveRecords("Personal Wire In Cheque Out Details.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out Details.IMD")
	Set task = db.Summarization
		task.AddFieldToSummarize "ACCOUNT_NUMBER"
		task.AddFieldToTotal "TRANSACTION_AMOUNT"
...
If haveRecords("Personal Wire In Cheque Out Summary.IMD") And  haveRecords("Credit Total.IMD") And haveRecords("Debit Total.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out Summary.IMD")
	Set task = db.VisualConnector
		id0 = task.AddDatabase ("Personal Wire In Cheque Out Summary.IMD")
		...
		task.CreateVirtualDatabase = False
		dbName = "Personal Wire In Cheque Out Summary Final.IMD"
...
Function K_Export
If haveRecords("Personal Wire In Cheque Out Summary Final.IMD") Then
	Set db = Client.OpenDatabase("Personal Wire In Cheque Out Summary Final.IMD") 
	Set task = db.ExportDatabase
		...
		eqn = ""
		task.PerformTask Client.WorkingDirectory &  "Reports\Personal Wire In Cheque Out Summary.MDB", "Database", "MDB2000", 1, db.Count, eqn
	Set task = Nothing
	Set db = Nothing
End if
...
If haveRecords("Personal Wire In Cheque Out Details.IMD") Then
	...
	Set task = db.ExportDatabase
		...
		task.AddFieldToInc "RUN_DATE"
		eqn = ""
		task.PerformTask Client.WorkingDirectory & "Reports\Personal Wire In Cheque Out Details.MDB", "Database", "MDB2000", 1, db.Count, eqn
	Set task = Nothing
	Set db = Nothing
End if
End Function
...
Function Z_CleanUp
	DeleteFile ("Personal Wire In Cheque Out.IMD")
	DeleteFile ("First Personal Wire In.IMD")
	DeleteFile ("Personal Wire In Cheque Out First WireIn.IMD")
	DeleteFile ("Personal Cheque Out After Wire In.IMD")
	DeleteFile ("Personal Wire In Cheque Out Details.IMD")
	DeleteFile ("Personal Wire In Cheque Out Summary.IMD")
	DeleteFile ("Personal Wire In Cheque Out Summ.IMD")
	DeleteFile ("Personal Wire In Cheque Out Temp.IMD")
	DeleteFile ("Personal Wire In Cheque Out Temp2.IMD")
	DeleteFile("Personal Wire In Cheque Out Summary Final.IMD")

...
' 	Objective:	
' 	Frequency:	
' 	Last Modified:	12/28/2014 3:57:38 PM
'====================================================================================================
'	Script Dependencies:
'====================================================================================================
...
Const RESULT_FILENAME = "TM4"
Const scriptname_log ="TM4.iss"
...
Client.CloseAll
	...
	Call ASummDailyTransRP()	'Summ Daily Repo Transactions
	Call BExtResult_Summ()		'Extract Transactions above Threshold at Summary
	Call CJoinExtResult_Dtl()		'Join to Extract Transactions above Threshold at Detail
...
Function ASummDailyTransRP
'If haveRecords("Daily_Transactions_Today.IMD") Then
'	Set db = Client.OpenDatabase("Daily_Transactions_Today.IMD")
If haveRecords("Weekly_History.IMD") Then
	Set db = Client.OpenDatabase("Weekly_History.IMD")
...
'	task.Criteria = "TRANSACTION_TYPE == ""REPO"" .AND. @Left(RECEIPT_NUMBER,2) == ""OW""  .AND. @Left(RECEIPT_NUMBER,5) <> ""OWPTR""  "
	task.Criteria = "TRANSACTION_TYPE == ""REPO"" .AND.( TRANSACTION_CHANNEL == ""WIRE""  .OR.  (PAYMENT_TYPE == ""E"" .AND. TRANSRC == ""IC"")) .AND.TRANSACTION_BRANCH == ""100"" "
	dbName = "Summ_DAILY_TRAN_TM4.IMD"
...
If haveRecords("Summ_DAILY_TRAN_TM4.IMD") Then
	Set db = Client.OpenDatabase("Summ_DAILY_TRAN_TM4.IMD")
	Set task = db.Extraction
	task.IncludeAllFields
	dbName = "TM4_Summ.IMD"
	task.AddExtraction dbName, "", " TRANSACTION_AMOUNT_SUM  >= "& e_TM4_Trans_Thres
...
Function CJoinExtResult_Dtl
'If haveRecords("Daily_Transactions_Today.IMD") Then
'	Set db = Client.OpenDatabase("Daily_Transactions_Today.IMD")
If haveRecords("Weekly_History.IMD") And haveRecords("TM4_Summ.IMD") Then
	Set db = Client.OpenDatabase("Weekly_History.IMD")
	Set task = db.JoinDatabase
	task.FileToJoin "TM4_Summ.IMD"
...
If haveRecords("TM4_SUMM.IMD") Then
	Set db = Client.OpenDatabase("TM4_SUMM.IMD")
...
If haveRecords("TM4_DTLS.IMD") Then
Set db = Client.OpenDatabase("TM4_DTLS.IMD")
	...
	task.AddFieldToInc "BRANCH_NAME"	
	eqn = ""
	task.PerformTask Client.WorkingDirectory & "Reports\TM4_DTLS.MDB", "Database", "MDB2000", 1, db.Count, eqn
	Set db = Nothing
End If
	Set task = Nothing
...
Set db = Client.OpenDatabase("TM4_Summ.IMD")
	...
	task.AddFieldToInc "BRANCH_NAME"	
	eqn = ""
	task.PerformTask Client.WorkingDirectory & "Reports\TM4_Summ.MDB", "Database", "MDB2000", 1, db.Count, eqn
	Set db = Nothing
End If
	Set task = Nothing
...
Function HCleanUp
	DeleteFile("Summ_DAILY_TRAN_TM4.IMD")
	DeleteFile("TM4_SUMM.IMD")
	DeleteFile("TM4_DTLS.IMD")	
End Function
